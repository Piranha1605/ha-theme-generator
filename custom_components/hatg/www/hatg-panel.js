...
class HATGPanel extends HTMLElement {
    ...
    return `
      ...
      <div class="modal-box">
        ...
        <div class="modal-actions">
          ...
          <button type="button" class="modal-btn primary" data-wallpaper-upload ${dialog.dataUrl && !dialog.uploading ? "" : "disabled"}>${dialog.uploading ? "Lädt hoch …" : "Hochladen & übernehmen"}</button>
        </div>
      </div>`;
  }

  openWallpaperDialog() {
    this._state.wallpaperDialog = { fileName: null, dataUrl: null, uploading: false, error: null };
    this.render();
  }
  ...
  async uploadWallpaper() {
    ...
    try {
      ...
      dialog.error = `Hochladen fehlgeschlagen: ${error && error.message ? error.message : "unbekannter Fehler"}`;
      this.render();
    }
  }
  ...
  bindEvents() {
    ...
    this.shadowRoot.querySelector("[data-wallpaper-file-input]")?.addEventListener("change", (event) => {
      ...
      reader.onload = () => {
        dialog.dataUrl = String(reader.result || "");
