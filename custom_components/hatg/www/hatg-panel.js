...
// Marker-System fuer die Cardmod-Vorlagen (siehe HATG_CARDMOD_TEMPLATES weiter
// unten): jede Vorlage wird beim Aktivieren als eigener, klar markierter Block
// an card-mod-root-yaml angehaengt und beim Deaktivieren wieder exakt anhand
// dieser Marker entfernt - bereits vorhandener Inhalt (z. B. von Hand
// geschriebenes CSS wie der Separator-Fix) bleibt dabei unangetastet, weil nur
// Text zwischen genau diesem Start-/End-Marker-Paar angefasst wird.
...
function hatgCardmodBlockRegex(id) {
  const start = hatgRegexEscape(hatgCardmodMarkerStart(id));
  const end = hatgRegexEscape(hatgCardmodMarkerEnd(id));
  return new RegExp(`\\n?${start}[\\s\\S]*?${end}`, "g");
}
function hatgCardmodBlockActive(text, id) {
  return String(text || "").includes(hatgCardmodMarkerStart(id));
}
function hatgRemoveCardmodBlock(text, id) {
  return String(text || "")
    .replace(hatgCardmodBlockRegex(id), "")
    .trim();
}
function hatgAppendCardmodBlock(text, id, css) {
  const base = hatgRemoveCardmodBlock(text, id);
  const block = `${hatgCardmodMarkerStart(id)}\n  ${String(css).trim()}\n  ${hatgCardmodMarkerEnd(id)}`;
  return base ? `${base}\n\n  ${block}` : `  ${block}`;
}

// 6 fertige, per Klick global im Theme aktivierbare Card-mod-Bausteine.
// Schreiben (als markierter Block) in card-mod-root-yaml, weil card_mod von
// dort aus per :has() auch in Bubble Cards aeussere ha-card-Huelle greifen
// kann (erprobtes Muster, siehe der bereits vorhandene Separator-Fix) -
// card-mod-card-yaml erreicht Bubble Cards eigenes, verschachteltes
// Shadow-DOM dagegen nachweislich nicht. Jede Vorlage enthaelt deshalb sowohl
// eine generische ha-card-Regel (native/Mushroom-Karten) als auch eine
// :has(.bubble-container)-Variante (Bubble Card, Container-Ebene). Tiefer
// verschachtelte Bubble-Elemente (z. B. einzelne Sub-Buttons) sind darüber
// nicht erreichbar - das ist eine bekannte Grenze von card_mod bei Bubble
// Card, kein Bug dieser Vorlagen. Ueberlappende Eigenschaften (z. B.
// box-shadow bei mehreren gleichzeitig aktiven Vorlagen) gewinnen nach
// CSS-Kaskade die zuletzt aktivierte Vorlage - Feintuning folgt spaeter.
const HATG_CARDMOD_TEMPLATES = [
  {
    id: "icon-farbe-hintergrund",
    label: "Icon: Farbe & Hintergrund-Chip",
    desc: "Icons bekommen einen runden, halbtransparenten Hintergrund-Chip in der Theme-Akzentfarbe statt einer nackten Glyphe - bei nativen/Mushroom-Karten per direkter CSS-Regel, bei Bubble Card über die eigenen --bubble-icon-*-Variablen (die als CSS-Custom-Properties zuverlässig auch in verschachteltes Shadow-DOM durchsickern).",
    css: `ha-card state-badge, ha-card .icon {
  ...
  },
  {
    ...
    desc: "Jede Karte im Dashboard bekommt statt einer flachen Fläche einen dezenten Verlauf von der eigenen Kartenfarbe zu einem leichten Schwarzschleier - wirkt räumlicher, ohne die Grundfarbe zu verändern.",
    css: `ha-card {
  ...
  ha-card:has(.bubble-container) {
    background-image: linear-gradient(160deg, var(--bubble-card-background-color, var(--card-background-color)) 0%, rgba(0, 0, 0, 0.12) 100%) !important;
  }`,
  },
  {
    id: "glow-schatten",
    label: "Glow / Leucht-Schatten",
    desc: "Weicher, farbiger Leuchtschatten in der Akzentfarbe rund um jede Karte statt eines neutralen Standard-Schattens - hebt das ganze Dashboard sanft hervor.",
    css: `ha-card {
  ...
  },
  {
    ...
    desc: "Dünner, akzentfarbener Rahmen mit leichter innerer Leuchtkante statt einer neutralen Standardkante - hebt jede Karte klar vom Hintergrund ab.",
    css: `ha-card {
    border: 1px solid color-mix(in srgb, var(--accent-color) 55%, transparent) !important;
    box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--accent-color) 20%, transparent) !important;
  }
  ha-card:has(.bubble-container) {
    border: 1px solid color-mix(in srgb, var(--accent-color) 55%, transparent) !important;
  }`,
  },
  {
    ...
    desc: "Milchiges, angehauchtes Glas mit Weichzeichner-Unschärfe statt blickdichter Kartenfarbe - global auf jeder Karte im Dashboard statt nur in einer einzelnen Karte (siehe die Glaseffekt-Plugins auf der Plugins-Seite für Einzelkarten-Varianten).",
    css: `ha-card {
    background: rgba(255, 255, 255, 0.10) !important;
  ...
  },
  {
    ...
    desc: "Mehrschichtiger Schatten (außen weich, innen eine feine Glanzkante oben) lässt jede Karte leicht erhaben statt flach wirken - dezenter Tiefeneffekt fürs ganze Dashboard.",
    css: `ha-card {
...
const HATG_TAIL_NAV = [
  ...
  { id: "cardmod-vorlagen", label: "Cardmod-Vorlagen", icon: "mdi:auto-fix" },
...
const HATG_DERIVE_RULES = {
  ...
  "card-background-color": [
    ...
    // Alle weiteren Bubble-Card-Hintergrundfelder je Kartentyp (Buttons,
    // Sub-Buttons, Separator, Popup, Horizontal Buttons Stack, Climate,
    // Cover, Media Player, Calendar, Select) - vorher komplett unabhaengig
    // von card-background-color, auf Wunsch jetzt einheitlich mitgezogen.
    // Bewusst ausgenommen: bubble-line-background-color/bubble-backdrop-
    // background-color (rgba-Overlays, keine Kartenflaeche), bubble-event-
    // background-image (Bild-Keyword, keine Farbe), bubble-button-active-
    // background-color/bubble-sub-button-active-background-color (haengen
    // bereits an accent-color bzw. sind bewusste Aktiv-Zustandsfarben).
    ...
    { key: "bubble-icon-background-color", transform: "copy" },
    ...
    { key: "bubble-separator-icon-background-color", transform: "copy" },
    ...
    { key: "bubble-cover-icon-background-color", transform: "copy" },
    ...
    { key: "bubble-media-player-icon-background-color", transform: "copy" },
    ...
    { key: "bubble-button-icon-background-color", transform: "copy" },
    ...
    { key: "bubble-select-icon-background-color", transform: "copy" },
...
// Startseite: "Status An"/"Status Aus" Master-Farbfelder - steuern gebündelt alle
// state-*-Felder, die hatgExtractStatusCategory() als "on" bzw. "off" klassifiziert.
const HATG_STATUS_AN_KEYS = [
...
const HATG_STATUS_AUS_KEYS = ["state-inactive-color", "state-off-color", "state-alert-off-color"];
const HATG_STATUS_MASTER_GROUPS = { an: HATG_STATUS_AN_KEYS, aus: HATG_STATUS_AUS_KEYS };
...
function hatgIsLong(key, value) {
  ...
  if (key === "card-mod-root-yaml") return true;
...
class HATGPanel extends HTMLElement {
  constructor() {
    ...
    this._state = {
      ...
      statusMasterFormat: { an: "hex", aus: "hex" },
    };
  ...
  sectionTitle() {
    ...
    if (this._activeSection === "cardmod-vorlagen") return "Cardmod-Vorlagen";
  ...
  renderStatusMasterRow(group, label) {
    const keys = this.getStatusMasterKeys(group);
    const rawValue = this.getStatusMasterRawValue(group);
    if (!this._state.statusMasterFormat) this._state.statusMasterFormat = { an: "hex", aus: "hex" };
    ...
    const otherLabel = otherMode === "dark" ? "Dark" : "Light";
    const copyButton = `
          <button class="apply-button copy-mode-button" type="button" data-status-master-copy="${group}" aria-label="Nach ${otherLabel} übernehmen" title="Status ${label} nach ${otherLabel} übernehmen">
            <ha-icon icon="mdi:swap-horizontal"></ha-icon>
          </button>`;
    const formatSwitch = `
          <span class="format-switch" role="group" aria-label="Status ${label} Anzeigeformat">
    ...
    const fieldCountTag = `<small class="format-tag">${keys.length} Felder</small>`;
    ...
    if (viewFmt === "rgba") {
      ...
      return `
          <div class="field-row" data-status-master-row="${group}">
            <span class="field-key">Status ${label}${fieldCountTag}</span>
            ${formatSwitch}
            <span class="field-input color-input-wrap rgba-input-wrap">
              <input class="native-color" type="color" value="${parsed.hex}" data-status-master-picker="${group}" aria-label="Status ${label}" />
              <input class="hex-input rgba-raw-input" type="text" spellcheck="false" data-status-master-hex="${group}" value="${hatgEscape(hatgComposeRgba(parsed.hex, parsed.alpha))}" aria-label="Status ${label} rgba-Wert" />
            </span>
            ${copyButton}
          </div>`;
    }
    ...
    return `
          <div class="field-row" data-status-master-row="${group}">
            ...
            ${formatSwitch}
            <span class="field-input color-input-wrap">
              <input class="native-color" type="color" value="${swatchHex}" data-status-master-picker="${group}" aria-label="Status ${label}" />
              <input class="hex-input" type="text" value="${displayValue}" spellcheck="false" data-status-master-hex="${group}" data-hex-format="${viewFmt}" aria-label="Status ${label} Farbcode" />
            </span>
            ${copyButton}
          </div>`;
  }
  ...
  renderStatusMasterSection() {
    ...
    return `
      <div class="editing-surface ${dark ? "editing-surface-dark" : ""}">
        <div class="field-group-title">Status-Farben (An/Aus)</div>
        <div class="field-list">
          ${this.renderStatusMasterRow("an", "An")}
          ${this.renderStatusMasterRow("aus", "Aus")}
        </div>
      </div>`;
  }
  ...
  toggleCardmodTemplate(id) {
    ...
    if (!tpl) return;
    const targetKey = "card-mod-root-yaml";
    const currentMode = this._state.editorMode;
    const wasActive = hatgCardmodBlockActive(this.currentValues()[targetKey] || "", id);
    ["light", "dark"].forEach((mode) => {
      this._state.editorMode = mode;
      const existing = String(this.currentValues()[targetKey] || "");
      const updated = wasActive
        ? hatgRemoveCardmodBlock(existing, id)
        : hatgAppendCardmodBlock(existing, id, tpl.css);
    ...
    this.showToast(
      wasActive
        ? `"${tpl.label}" deaktiviert und aus card-mod-root-yaml entfernt.`
        : `"${tpl.label}" aktiviert - global ins Theme (card-mod-root-yaml) übernommen.`
    );
  }

  renderCardmodTemplates() {
    const activeText = String(this.currentValues()["card-mod-root-yaml"] || "");
    ...
    return `
      <section class="editor-section">
        <div class="section-heading">
          ...
          <p>Fertige card-mod-Bausteine, die per Klick global im Theme aktiviert werden (landen markiert in <code>card-mod-root-yaml</code>, für Light und Dark gleichzeitig) - kein Kopieren/Einfügen nötig. Wirkt auf native/Mushroom-Karten zuverlässig vollständig, bei Bubble Card auf Container-Ebene (Hintergrund, Rahmen, Schatten, Icon-Farbe); einzelne tief verschachtelte Bubble-Elemente wie Sub-Buttons erreicht das technisch bedingt nicht - dafür die Kopiervorlagen auf der Plugins-Seite nutzen. Mehrere Vorlagen lassen sich gleichzeitig aktivieren; bei überlappenden Eigenschaften (z. B. Schatten) gewinnt die zuletzt aktivierte.</p>
        </div>
  ...
  buildYamlText(rootNameOverride) {
    ...
    // card_mod liest seine Spezialfelder (card-mod-theme, card-mod-root-yaml,
    // card-mod-card-yaml und jede weitere "card-mod-*"-Variable) NUR von der
    // FLACHEN obersten Ebene des Theme-Objekts - niemals aus modes.light/dark.
    // Frueher hat HATG ausnahmslos alles (auch diese Felder) unter modes:
    // verschachtelt, wodurch card_mod sie nie gesehen hat. Deshalb werden
    // card-mod-*-Schluessel hier gesammelt und einmalig auf oberster Ebene
    // ausgegeben (Light gewinnt bei unterschiedlichen Werten, sonst Dark).
    const isCardMod = (key) => /^card-mod-/.test(key);
    const flatCardMod = {};
    const noteFlat = (key, value) => {
      if (value === undefined || value === "") return;
      if (!(key in flatCardMod)) flatCardMod[key] = value;
    };
    ...
    const modeBlock = (mode) => {
      ...
      HATG_MANIFEST.sections.forEach((s) => {
        ...
        groups.forEach((g) => {
          ...
          normalKeys
            .filter((k) => isCardMod(k) && values[k] !== undefined && values[k] !== "")
            .forEach((k) => noteFlat(k, k === "card-mod-theme" ? name : values[k]));
          const nestedKeys = normalKeys.filter((k) => !isCardMod(k));
          ...
          if (g.keys.includes(HATG_CUSTOM_YAML_KEY)) {
            ...
            // key: value-Paare zu echten Geschwister-Theme-Variablen. Zeilen,
            // die selbst mit "card-mod-" beginnen, wandern stattdessen auf die
            // flache oberste Ebene (siehe oben) statt unter modes:.
      ...
      if (extra) {
        // Schluessel, die bereits ueber "eigene-theme-eintraege" exportiert
        // wurden (z. B. nach einem Reimport der eigenen HATG-Ausgabe), werden
        // hier nicht nochmal als "nicht erkannt" dupliziert.
    ...
    const flatKeys = Object.keys(flatCardMod);
    const flatBlock = flatKeys.length
      ? `  # card-mod (theme-global - card_mod liest diese Felder nur auf oberster Ebene, nie aus modes.light/dark)\n${flatKeys.map((k) => `  ${k}: ${hatgQuoteYamlValue(flatCardMod[k])}`).join("\n")}\n`
      : "";
  ...
  renderMainSection() {
    ...
    if (this._activeSection === "cardmod-vorlagen") return this.renderCardmodTemplates();
  ...
  bindEvents() {
    ...
    this.shadowRoot.querySelectorAll("[data-toggle-cardmod-template]").forEach((el) => {
      el.addEventListener("click", () => {
        const id = el.getAttribute("data-toggle-cardmod-template");
        if (id) this.toggleCardmodTemplate(id);
      });
    });
  ...
  getStatusMasterKeys(group) {
    return group === "aus" ? HATG_STATUS_AUS_KEYS : HATG_STATUS_AN_KEYS;
  }
  ...
  getStatusMasterHex(group, mode) {
    const raw = this.getStatusMasterRawValue(group, mode);
    return hatgIsHex(raw) ? hatgNormalizeHex6(raw) : hatgParseRgba(raw).hex;
  }

  applyStatusMasterColor(group, value) {
    const keys = this.getStatusMasterKeys(group);
    keys.forEach((key) => this.commitField(key, value));
    this.applyPreviewTheme();
  }
  ...
  copyStatusMasterToOtherMode(group) {
    ...
    this._state.editorMode = currentMode;
    this.render();
    const label = group === "aus" ? "Aus" : "An";
    this.showToast(`Status ${label} nach ${otherMode === "dark" ? "Dark" : "Light"} übernommen: ${value}.`);
  }
  ...
  applyImportedTheme(parsed) {
    ...
    hatgNormalizeRgbaLegacyHex(this._state.values.light);
    hatgNormalizeRgbaLegacyHex(this._state.values.dark);
    // Ein Import ist ein FRISCHER Ladevorgang des gewaehlten Themes, kein
    // Zusammenfuehren mit dem vorherigen Zustand: unbekannte Zusatzfelder aus
    // einem FRUEHEREN Import/Test duerfen nicht liegen bleiben und in jeden
    // danach importierten/gespeicherten Theme erneut mit exportiert werden
    // (das fuehrte dazu, dass eine laengst irrelevante alte card-mod-Leiche
    // immer wieder in voellig neuen, eigentlich card-mod-freien Themes auftauchte).
