...
// v0.2.23: Enrico wollte eigene Themes (u.a. eine iOS- und eine Android-
// Vorlage nach den jeweils offiziellen Plattform-Vorgaben) als vollwertige
// Basis-Vorlage hinterlegen koennen. Die alte Vorlage kannte nur die 7
// Kern-Farben oben - Radius und Schatten blieben immer beim aktuellen
// Manifest-Standardwert, egal welche Vorlage man waehlte. Damit eine
// Vorlage wirklich den kompletten "Look" transportiert (nicht nur die
// Farbe), kommen hier alle Radius-/Schatten-Felder aus HA-Grundgerüst und
// Bubble Card sowie die Schriftart-Felder dazu.
...
// Schriftart ist bei einer Plattform-Vorlage (iOS/Android) eines der
// staerksten optischen Unterscheidungsmerkmale (SF-Pro-Stack vs. Roboto) -
// zaehlt daher ebenfalls zum uebernommenen "Look" einer Vorlage dazu.
...
// Enricos eigenes v0219-Theme (hochgeladen als v0219(3).yaml): warmer,
// mehrschichtiger "Neumorphic"-Schatten (Lichtkante oben, Schattenkante
// unten) - hell auf Bernstein/Braun-Basis getoent, dunkel auf Schwarz-Basis,
// identisch auf fast allen Karten-/Popup-/Button-Ebenen wiederverwendet.
...
// bubble-select-box-shadow/bubble-footer-box-shadow im Dark-Mode nutzen in
// Enricos Datei bewusst einen einfacheren Wert statt des grossen Neumorphic-
// Schattens (so 1:1 aus der hochgeladenen Datei uebernommen, keine eigene
// Vereinheitlichung).
...
// iOS-Vorlage: Apples "Materials"-Prinzip statt Schlagschatten (Apple HIG,
// Foundations/Materials: in Light Mode ein einfacher, dezenter Schatten,
// in Dark Mode setzt Apple bewusst auf hellere Flaechenfarben statt Schatten
// fuer erhabene Inhalte, da Schatten auf dunklem Grund kaum wirken) - daher
// im Dark Mode nur ein sehr zurueckhaltender Schatten (ganz ohne Schatten
// waere in Bubble Card/HA optisch zu flach, da hier keine echte Vibrancy
// existiert). Eckenradius: Apple veroeffentlicht keine feste, numerierte
// Radius-Skala wie Material Design, sondern beschreibt "continuous corners"
// als Kurvenprinzip - die Werte unten sind die in der iOS-Entwicklung
// gaengigen, haeufig referenzierten Groessen (kleine Steuerelemente ~10pt,
// Karten ~14pt, Sheets/Popups/Widgets ~20pt, Icons/Buttons vollstaendig
// rund), keine von Apple 1:1 numerisch vorgeschriebene Tabelle.
...
// Android-Vorlage: Material Design 3 offizielle Shape-Skala (m3.material.io/
// styles/shape/corner-radius-scale: None 0, Extra-Small 4, Small 8,
// Medium 12, Large 16, Extra-Large 28, Full = voll rund) und offizielle
// Elevation-Schatten-Kurven (m2.material.io/design/environment/elevation,
// in M3 unveraendert weiterverwendet: 1dp fuer ruhende Karten, 6dp fuer
// Dialoge). Form je Bauteil nach den jeweiligen M3-Komponenten-Specs: Karte
// = Medium, Button/Icon-Button/FAB/Slider-Track = Full (M3s markantester
// Formwechsel gegenueber M2 - durchgehend pillenfoermig), Menue/Dropdown =
// Extra-Small, Dialog = Extra-Large, Chip = Small.
...
// Material Elevation 1dp (ruhende Karte) bzw. 6dp (Dialog) - offizielle,
// in Angular Material/MUI identisch implementierte CSS-Werte. Im Dark
// Theme empfiehlt Material selbst primaer eine hellere Flaechen-Tonstufe
// statt eines staerkeren Schattens (elevation overlay) - der Schatten
// selbst bleibt aber bestehen, nur mit hoeherer Deckkraft, da er auf
// dunklem Grund sonst kaum sichtbar waere.
...
const HATG_BASE_PRESETS = [
  ...
  },
  {
    ...
    desc: "Enricos eigenes v0219-Theme: Blaugrün-Akzent (#5BB6BE), warmer mehrschichtiger Neumorphic-Schatten, container-abhängige Eckenradien (12–18px, Icons vollständig rund).",
    swatch: "#5BB6BE",
    values: {
      light: hatgPickCoreValues("light", {
  ...
  },
  {
    ...
    values: {
      ...
      dark: hatgPickCoreValues("dark", {
        ...
        "accent-color": "#0A84FF",
        // Apple HIG (Materials): in Dark Mode traegt eine hellere Flaeche
        // die erhabene Karte, nicht der Schatten - Seite bewusst auf reinem
        // Schwarz (systemBackground dark), Karte auf dem etwas helleren
        // #1C1C1E (secondarySystemBackground dark).
...
// ===== Cardmod (v0.2.15): Baustein-Auswahl statt Blanket-CSS =====
// A/B-Test (hatg_final.yaml vs hatg(1).yaml, komplett identisch bis auf die
// entfernten card-mod-root-yaml/card-mod-card-yaml-Bloecke, live von Enrico
// im Dashboard verglichen: kein sichtbarer Unterschied) hat gezeigt, dass ein
// Grossteil des bisherigen card-mod-Textes nur dupliziert, was Bubble Card/
// Mushroom ohnehin nativ aus denselben flachen Theme-Variablen lesen. Statt
// weiter pauschal ALLES ins Theme zu schreiben, lassen sich hier einzelne
// CSS-Bausteine gezielt ein-/ausschalten (Cardmod-Seite, wie Plugins aber mit
// echtem Toggle statt Kopieren). Marker-Kommentare im card-mod-root-yaml/
// card-mod-card-yaml-Text selbst (HATG:CARDMOD:<id>:START/END) halten fest,
// welche Bausteine gerade aktiv sind - der Text bleibt dadurch die einzige
// Wahrheitsquelle (kein separater, aus dem Ruder laufender App-Zustand).
function hatgCardmodMarkerStart(id) {
  return `/* HATG:CARDMOD:${id}:START */`;
}
function hatgCardmodMarkerEnd(id) {
  return `/* HATG:CARDMOD:${id}:END */`;
}
function hatgCardmodBlock(cardmod, part) {
  const css = part === "root" ? cardmod.rootCss : cardmod.cardCss;
  if (!css) return "";
  return `${hatgCardmodMarkerStart(cardmod.id)}\n${css}\n${hatgCardmodMarkerEnd(cardmod.id)}`;
}
function hatgIsCardmodEnabled(text, id) {
  return typeof text === "string" && text.includes(hatgCardmodMarkerStart(id));
}
function hatgCardmodHeaderComment(part) {
  return part === "root"
    ? `/* Erstellt mit HATG v${HATG_VERSION} - card-mod-root-yaml, zusammengesetzt aus einzelnen Cardmod-Bausteinen (siehe Cardmod-Seite) */`
    : `/* Erstellt mit HATG v${HATG_VERSION} - card-mod-card-yaml, zusammengesetzt aus einzelnen Cardmod-Bausteinen (siehe Cardmod-Seite) */`;
}
// Baut den kompletten Feldinhalt (card-mod-root-yaml ODER card-mod-card-yaml)
// aus der Menge aktivierter Cardmod-IDs neu zusammen - immer in fester
// Registry-Reihenfolge, damit das Ergebnis deterministisch/diff-stabil bleibt.
function hatgBuildCardmodField(enabledIds, part) {
  const blocks = HATG_CARDMODS.map((c) => (enabledIds.has(c.id) ? hatgCardmodBlock(c, part) : "")).filter((b) => b);
  return [hatgCardmodHeaderComment(part), "", ...blocks].join("\n\n") + "\n";
}
// Liest aus bestehendem card-mod-root-yaml/card-mod-card-yaml-Text (z.B. nach
// Theme-Import), welche Cardmod-IDs gerade markiert/aktiv sind.
function hatgReadEnabledCardmodIds(rootText, cardText) {
  const enabled = new Set();
  HATG_CARDMODS.forEach((c) => {
    if (hatgIsCardmodEnabled(rootText, c.id) || hatgIsCardmodEnabled(cardText, c.id)) enabled.add(c.id);
  });
  return enabled;
}

const HATG_CARDMODS = [
  // ---- Standardmaessig aktiv: bestaetigte echte Bugfixes ----
  {
    id: "separator-fix",
    label: "Separator (Trenner ohne Kartenschatten)",
    desc: "Trenner-Karten (card_type: separator) wirken flach statt wie eine erhabene Karte, plus eigene Icon-/Text-/Linienfarben. Bestaetigter Bugfix (v0.2.13) - ohne diesen Baustein zeigt Bubble Card den normalen Kartenschatten auch bei Trennern.",
    system: "bubble",
    defaultOn: true,
    rootCss: `/* Separator */
  .bubble-separator {
  ...
  {
    id: "popup-header-fix",
    label: "Popup & Dialog-Header",
    desc: "Popup-Hauptbereich UND -Header (Name-/Icon-Leiste oben) unabhaengig voneinander einfaerbbar. Bestaetigter Bugfix (v0.2.13, Forum-Frage von lSi) - ohne diesen Baustein folgt der Header derselben Farbe wie normale Button-Karten auf dem Dashboard, nicht dem Popup selbst.",
    system: "bubble",
    defaultOn: true,
    rootCss: `/* Popup & Dialog */
  .bubble-pop-up,
  .bubble-pop-up-background,
  .bubble-pop-up-container {
    border-radius: var(--bubble-pop-up-border-radius, var(--bubble-border-radius, 18px)) !important;
  ...
  {
    id: "icon-glow-status",
    label: "Icon-Leucht-Zustand (An/Aus-Glühen)",
    desc: "Icon-Kreis glueht bei 'An' sanft in der Akzentfarbe, wirkt bei 'Aus' leicht eingedrueckt/matt. Bestaetigtes, global wirkendes Feature (v0.4.61) fuer alle Bubble-Karten mit Icon.",
    system: "bubble",
    defaultOn: true,
    rootCss: `/* Status */
  .bubble-icon-container {
  ...
  },
  {
    ...
    label: "Horizontal-Buttons-Stack (inkl. Button-Felder)",
    desc: "Eigene Radius-/Schatten-/Hintergrundfarbe fuer die Buttons-Leiste, plus jetzt endlich verdrahtet: die 3 bisher toten Felder fuer Icon-, Text- und Hintergrundfarbe JE BUTTON (bubble-horizontal-buttons-stack-button-icon-color/-text-color/-background-color). Bestaetigter Bugfix (v0.4.27), Verdrahtung der 3 Button-Felder neu in v0.2.15.",
    system: "bubble",
    defaultOn: true,
    rootCss: `/* Horizontal Buttons Stack */
  .bubble-horizontal-buttons-stack-card-container {
    border-radius: var(--bubble-horizontal-buttons-stack-border-radius, var(--bubble-border-radius, 18px)) !important;
    box-shadow: var(--bubble-horizontal-buttons-stack-box-shadow, var(--bubble-box-shadow, none)) !important;
    background: var(--bubble-horizontal-buttons-stack-background-color, var(--bubble-main-buttons-background-color, var(--card-background-color))) !important;
  ...
  {
    id: "mushroom-bridge",
    label: "Mushroom-Icon-Brücke",
    desc: "Mushroom-Karten uebernehmen automatisch dieselbe Icon-Farbe/-Hintergrund/-Form wie Bubble Card, statt Mushrooms eigenen (oft abweichenden) Default. Bestaetigter Bugfix - siehe HATG_FARBZUORDNUNG.md.",
    system: "mushroom",
    defaultOn: true,
    rootCss: `/* Mushroom */
  mushroom-shape-icon {
  ...
  // ---- Standardmaessig AUS: laut A/B-Test vermutlich redundant (Bubble Card/
  // Mushroom lesen dieselben flachen Theme-Variablen ohnehin nativ) ----
  {
    id: "base-card",
    label: "Basis-Karte (Radius/Hintergrund/Rahmen/Schatten)",
    desc: "Setzt Kartenradius/-hintergrund/-rahmen/-schatten fuer alle Bubble-Kartentypen erneut per card_mod. A/B-Test (v0.2.14): kein sichtbarer Unterschied beim Entfernen - Bubble Card liest diese Werte vermutlich schon selbst aus denselben Variablen. Optional, falls doch ein Randfall auftaucht.",
    system: "bubble",
    defaultOn: false,
    rootCss: `/* Karte */
  .bubble-container,
  ...
  .bubble-pop-up-container {
    ...
    -webkit-backdrop-filter: var(--card-backdrop-blur, none) !important;
  ...
  {
    id: "icon-base-colors",
    label: "Icon-Grundfarben",
    desc: "Icon-Kreis-Hintergrund/-Farbe erneut per card_mod gesetzt. A/B-Test: kein sichtbarer Unterschied beim Entfernen.",
    system: "bubble",
    defaultOn: false,
    rootCss: `.bubble-icon-container,
  .bubble-icon-wrapper,
  ...
  {
    id: "name-state-text",
    label: "Name/Status-Text-Grundfarben",
    desc: "Name- und Status-Textfarbe/-groesse erneut per card_mod gesetzt. A/B-Test: kein sichtbarer Unterschied beim Entfernen.",
    system: "bubble",
    defaultOn: false,
    rootCss: `.bubble-name,
  ...
  {
    id: "sub-buttons",
    label: "Sub-Buttons-Grundstil",
    desc: "Sub-Button-Radius/-Hintergrund/-Textfarbe/-Schatten erneut per card_mod gesetzt. A/B-Test: kein sichtbarer Unterschied beim Entfernen.",
    system: "bubble",
    defaultOn: false,
    rootCss: `/* Sub-Buttons */
  .bubble-sub-button,
  ...
  {
    id: "switch-slider",
    label: "Schalter/Slider-Grundstil",
    desc: "Slider-Leiste (Track/Fuellung) erneut per card_mod gesetzt. A/B-Test: kein sichtbarer Unterschied beim Entfernen - Bubble Card re-injiziert beim Ziehen ohnehin eigenes CSS mit hoeherer Prioritaet (siehe Plugin-Seite/Slider-Plugins).",
    system: "bubble",
    defaultOn: false,
    rootCss: `/* Schalter, Toggle & Slider */
  .bubble-range,
  .bubble-range-slider,
  ...
  {
    id: "climate-cover",
    label: "Climate/Cover-Grundstil",
    desc: "Eigene Radius-/Schatten-/Hintergrundfarbe fuer Climate- und Cover-Karten erneut per card_mod gesetzt. A/B-Test: kein sichtbarer Unterschied beim Entfernen.",
    system: "bubble",
    defaultOn: false,
    rootCss: `/* Climate */
  .bubble-climate-card-container {
  ...
  {
    id: "media-player",
    label: "Media-Player-Grundstil",
    desc: "Eigene Radius-/Schatten-/Hintergrundfarbe fuer Media-Player-Karten erneut per card_mod gesetzt. A/B-Test: kein sichtbarer Unterschied beim Entfernen.",
    system: "bubble",
    defaultOn: false,
    rootCss: `/* Media Player */
  .bubble-media-player {
...
// Cardmod-Seite: Enrico - "und ein icon dazu für welches system das ist bubble
// mushroom oder ha" - jeder Baustein bekommt sein "system"-Feld (siehe
// HATG_CARDMODS), dieses Mapping liefert Icon/Label/Farbe dafuer. Icon und
// Farbverlauf sind bewusst identisch zu den echten Sektionen in der
// Seitenleiste (bubble-card/mushroom/hintergruende-karten), damit dieselbe
// Zuordnung wie im Hauptmenue sofort wiedererkennbar ist.
const HATG_CARDMOD_SYSTEM_META = {
  bubble: { icon: "mdi:circle-multiple-outline", label: "Bubble Card", navId: "bubble-card" },
  mushroom: { icon: "mdi:mushroom-outline", label: "Mushroom", navId: "mushroom" },
  ha: { icon: "mdi:view-dashboard-outline", label: "Home Assistant (generisch)", navId: "hintergruende-karten" },
};
...
// v0.2.19: Enrico - "alle felder muss hoch unter mushroom" - eigener Nav-
// Eintrag, damit er im Hauptmenue direkt nach Mushroom eingehaengt werden
// kann (siehe renderNavigation()), statt fest unten in den Tools zu haengen.
...
const HATG_TAIL_NAV = [
  ...
  { id: "cardmod", label: "Cardmod", icon: "mdi:tune-variant" },
  // "generatoren" (Blur & Kartentransparenz) vorerst deaktiviert - Live-Test-
  // Rueckmeldung: "der generator geht nicht.schalte den erst einmal ab" (v0.2.3).
...
function hatgTokenizeKey(key) {
  return String(key ?? "")
    ...
    // nach "color" oder "graph". Feedback: "filter wie 1,2,3,4 ... können raus".
    .filter((t) => !/^\d+$/.test(t));
}
...
function hatgParseThemeYaml(text, knownKeys) {
  ...
  for (; i < lines.length; i++) {
    ...
    if (!currentMode) {
      // Klassisches HA-Theme ohne modes:/light:/dark:-Aufteilung - alle Felder
      // stehen direkt unter dem Root-Key und gelten fuer beide Modi gleich.
  ...
  if (!result.foundLight && !result.foundDark) {
    ...
    // v0.2.13: Bugfix - HATG selbst kann Themes ohne modes:/light:/dark:
    // exportieren (z. B. bewusst umgestellt auf "nur Dark"), konnte sie aber
    // nicht wieder einlesen und zeigte faelschlich "Keine light:/dark:-
    // Abschnitte gefunden". Ein flaches Theme (Felder direkt unter dem
    // Root-Key) gilt in HA fuer beide Farbschemata gleich - deshalb hier auf
    // light UND dark uebernehmen, statt einen Fehler zu werfen.
    result.light = { ...flat };
    result.dark = { ...flat };
...
class HATGPanel extends HTMLElement {
  ...
  sectionTitle() {
    ...
    if (this._activeSection === "cardmod") return "Cardmod";
  ...
  renderNavigation() {
    ...
    HATG_MANIFEST.sections.forEach((s) => {
      // card-mod-generator ist seit v0.2.19 keine eigene Seite mehr - die
      // Felder (card-mod-theme/card-mod-card-yaml/card-mod-root-yaml/
      // card-backdrop-blur) leben jetzt am Ende der Code-Editor-Seite
      // (siehe renderCodeEditor()), da es inhaltlich ohnehin dasselbe ist.
      ...
      // "Alle Felder" direkt unter Mushroom im Hauptmenue statt unten in Tools.
  ...
  renderFieldRow(key) {
    ...
    // Code anpassen"-Feld, das sich der Nutzer gewuenscht hat.
    ...
    if (originalFormat === "hex" || originalFormat === "rgba") {
      ...
      // v0.2.13: Rueckmeldung - "wenn ich einen rgba wert eingebe springt er zurueck
      // zum hex wert...ich sehe dann nicht welche felder wirklich rgb oder
      // rgba haben." Ursache: die Pillen-Vorauswahl hing nur am statischen
      // Manifest-Default des Feldes (originalFormat), nicht am tatsaechlich
      // gespeicherten Wert - ein Feld mit Hex-Default zeigte also selbst dann
      // "Hex" an, wenn der aktuelle Wert laengst ein echtes rgba(...) war.
      // Jetzt: ohne explizite manuelle Auswahl richtet sich die Pille nach dem
      // wirklichen, aktuellen Wert.
  ...
  renderSectionToolbar(keys) {
    // v0.2.19: Enrico - die Mehrfachauswahl/Bulk-Sync auf einzelnen Sektions-
    // seiten war "das alte Feld" von vor der ausgebauten "Alle Felder"-Seite
    // (Suche + Tag-Filter + Bulk-Sync über ALLE Felder hinweg, jetzt direkt
    // unter Mushroom im Hauptmenue). Da "Alle Felder" dieselbe Aufgabe bereits
    // vollstaendig abdeckt (und dort auch tatsaechlich funktioniert), faellt
    // der Auswaehlen-Button/die Bulk-Leiste auf normalen Sektionsseiten raus -
    // Mehrfachauswahl gibt es nur noch auf der "Alle Felder"-Seite selbst
    // (siehe renderAlleFelder(), die ihre eigene Auswaehlen-Leiste behaelt).
    return `
  ...
  renderCodeEditor() {
    ...
    const sectionCards = [];
    HATG_MANIFEST.sections.forEach((s) => {
      // v0.2.19: card-mod-generator bekommt hier unten einen eigenen,
      // vollwertigen Block (echte Widgets statt Quick-Edit-Text) - siehe
      // cardModBlock weiter unten.
      if (s.id === "card-mod-generator") return;
    ...
    // v0.2.19: Enrico - "die felder aus cardmod generator können ans ende der
    // seite code editor da das irgendiw das selbe ist." Card-mod & Generator
    // ist keine eigene Seite mehr (siehe renderNavigation()), die echten
    // Feldwidgets (YAML-Editor/Blur-Regler) landen jetzt hier ganz unten.
    const cardModMeta = HATG_MANIFEST.sections.find((s) => s.id === "card-mod-generator");
    const cardModBlock = cardModMeta
      ? `
        <div class="code-section-card code-section-card-cardmod">
          <div class="code-section-title">${hatgEscape(cardModMeta.label)}</div>
          ${this.renderFieldList(cardModMeta.keys, cardModMeta.groups)}
        </div>`
      : "";
    return `
      <section class="editor-section">
  ...
  renderPluginInfoDialog() {
    ...
    return `
      ...
      <div class="modal-box modal-box-wide">
        ...
        <p class="plugin-hint">${plugin.hint || ""}</p>
  ...
  // Cardmod (v0.2.15): card-mod-root-yaml/card-mod-card-yaml sind bei HATG in
  // light+dark textgleich (reine var()-Referenzen, keine hartcodierten Farben
  // - per diff bestaetigt). Der aktivierte Zustand wird deshalb bewusst nicht
  // in einem zusaetzlichen App-Zustand gehalten, sondern direkt aus dem Text
  // der aktuellen Werte gelesen (Marker-Scan) - der Text bleibt die einzige
  // Wahrheitsquelle, z.B. auch nach einem Theme-Import.
  cardmodEnabledIds() {
    const values = this.currentValues();
    return hatgReadEnabledCardmodIds(values["card-mod-root-yaml"] || "", values["card-mod-card-yaml"] || "");
  }

  toggleCardmod(id) {
    const cardmod = HATG_CARDMODS.find((c) => c.id === id);
    if (!cardmod) return;
    const enabled = this.cardmodEnabledIds();
    if (enabled.has(id)) enabled.delete(id);
    else enabled.add(id);
    const rootText = hatgBuildCardmodField(enabled, "root");
    const cardText = hatgBuildCardmodField(enabled, "card");
    // Bewusst in BEIDEN Modi gleichzeitig geschrieben (siehe Kommentar oben) -
    // unabhaengig davon, welcher Modus (Light/Dark) im Editor gerade aktiv ist.
    ["light", "dark"].forEach((mode) => {
      this._state.values[mode]["card-mod-root-yaml"] = rootText;
      this._state.values[mode]["card-mod-card-yaml"] = cardText;
      this._state.source[mode]["card-mod-root-yaml"] = "custom";
      this._state.source[mode]["card-mod-card-yaml"] = "custom";
    });
    this.render();
  }
  ...
  renderCardmods() {
    const enabled = this.cardmodEnabledIds();
    const cards = HATG_CARDMODS.map((cardmod) => {
      const isOn = enabled.has(cardmod.id);
      const sysMeta = HATG_CARDMOD_SYSTEM_META[cardmod.system] || HATG_CARDMOD_SYSTEM_META.ha;
      return `
        <div class="cardmod-card ${isOn ? "is-selected" : ""}">
          <label class="cardmod-toggle-switch">
            <input type="checkbox" data-cardmod-toggle="${cardmod.id}" ${isOn ? "checked" : ""} />
            <span class="cardmod-toggle-slider"></span>
          </label>
          <span class="cardmod-system-badge" style="background: ${hatgNavIconGradient(sysMeta.navId)};" title="${hatgEscape(sysMeta.label)}" aria-label="${hatgEscape(sysMeta.label)}">
            <ha-icon icon="${sysMeta.icon}"></ha-icon>
          </span>
          <strong>${hatgEscape(cardmod.label)}</strong>
          <button type="button" class="plugin-info-button" data-cardmod-info-open="${cardmod.id}" title="Beschreibung anzeigen" aria-label="Beschreibung anzeigen">
            <ha-icon icon="mdi:information-outline"></ha-icon>
          </button>
        </div>`;
    }).join("");
    return `
      <section class="editor-section">
        <div class="section-heading">
          <span class="eyebrow">Erweiterungen</span>
          <h1>Cardmod</h1>
          <p>Statt eines pauschalen card-mod-Blocks für alle Karten: hier einzelne CSS-Bausteine gezielt ein-/ausschalten (Details über das <ha-icon icon="mdi:information-outline"></ha-icon>-Symbol je Baustein). Aktive Bausteine landen sofort in card-mod-root-yaml/card-mod-card-yaml (Light+Dark gleichzeitig). Die ersten 5 sind bestätigte Bugfixes und standardmäßig an, der Rest ist laut A/B-Test vermutlich redundant und deshalb aus.</p>
        </div>
        <div class="cardmod-grid">${cards}</div>
      </section>`;
  }

  renderCardmodInfoDialog() {
    const id = this._state.cardmodInfoOpenId;
    if (!id) return "";
    const cardmod = HATG_CARDMODS.find((c) => c.id === id);
    if (!cardmod) return "";
    const sysMeta = HATG_CARDMOD_SYSTEM_META[cardmod.system] || HATG_CARDMOD_SYSTEM_META.ha;
    return `
      <div class="modal-scrim" data-cardmod-info-close></div>
      <div class="modal-box modal-box-wide">
        <h3>
          <span class="cardmod-system-badge" style="background: ${hatgNavIconGradient(sysMeta.navId)};" title="${hatgEscape(sysMeta.label)}" aria-label="${hatgEscape(sysMeta.label)}">
            <ha-icon icon="${sysMeta.icon}"></ha-icon>
          </span>
          ${hatgEscape(cardmod.label)}
        </h3>
        <p>${hatgEscape(cardmod.desc)}</p>
        <p class="plugin-hint">${
          cardmod.defaultOn
            ? "Standardmäßig aktiv – bestätigter Bugfix."
            : "Standardmäßig aus – laut A/B-Test vermutlich redundant (Bubble Card/Mushroom lesen die Werte ohnehin nativ), optional zuschaltbar."
        }</p>
        <div class="modal-actions">
          <button type="button" class="modal-btn primary" data-cardmod-info-close>Schließen</button>
        </div>
      </div>`;
  }
  ...
  generatorApplyCardTransparency(rgba) {
    ...
    // im echten Dashboard unlesbar (siehe Live-Test-Screenshot: Auswahlmenue-
    // Hintergrund wurde durchsichtig statt der Karten). Deshalb hier gezielt
    // nur echte Karten-Hintergrundfelder setzen.
    //
    // v0.2.7: Live-Test-Rueckmeldung (Screenshot Design-Dropdown in den HA-
  ...
  // Glaseffekt (v0.2.5): Wunsch - "wieso koennen wir zum Beispiel den
  ...
  // Hintergrund (v0.2.5): Wunsch - "das Thema Hintergrund brauchen wir auch
  ...
  backgroundStyleDefs() {
    return {
      ...
      // v0.2.20: "Radial" befuellt - dritter Verlauf-Preset, diesmal kreisfoermig
      // statt linear, mit einem Glanzpunkt aus der oberen linken Ecke statt
      // exakt mittig (wirkt weniger wie ein flacher Kreis, mehr wie ein
      // dezenter Lichteinfall). Bewusst ein fixer Farbpreset wie Sonnenaufgang/
      // Nachthimmel, nicht an accent-color gekoppelt - wer eine an die eigene
      // Akzentfarbe angepasste Version will, nimmt "Eigene" mit var(--accent-color).
  ...
  renderBackgroundPicker() {
    ...
    // v0.2.20: Enrico wollte fuer "Bild" ein eigenes, hochgeladenes Bild
    // statt eines Presets - Kachel ist jetzt aktiv (kein "soon" mehr) und
    // oeffnet renderWallpaperDialog() statt direkt einen Wert zu committen
    // (siehe [data-bg-style="bild"]-Sonderfall in bindEvents()). "Radial"
  ...
  // v0.2.21: Enrico meldete "Hochladen fehlgeschlagen: unbekannter Fehler" beim
  // Hochladen eines echten Fotos (Unsplash-JPG, mehrere MB). Ursache: Home
  // Assistants Websocket-Verbindung (aiohttp) hat ein Nachrichtengroessen-Limit
  // (Standard 4 MB) - ein als Base64 kodiertes Foto sprengt das leicht (Base64
  // blaeht die Groesse um ~33% auf). Die Verbindung bricht dann die Nachricht
  // ab, bevor unser eigener Backend-Code (der sonst eine klare Fehlermeldung
  // liefern wuerde) ueberhaupt erreicht wird - im Frontend kommt nur ein
  // rohes Fehler-/Close-Event ohne .message an, daher der nichtssagende
  // Fallback-Text. Fix: das Bild wird jetzt vor dem Hochladen client-seitig
  // ueber ein <canvas> verkleinert/neu komprimiert, wenn die Originaldatei
  // groesser als WALLPAPER_COMPRESS_THRESHOLD ist - danach passt es sicher
  // unters Limit. Kleine Bilder werden unveraendert wie bisher hochgeladen.
  ...
  async prepareWallpaperFile(file) {
    const COMPRESS_THRESHOLD_BYTES = 1.5 * 1024 * 1024; // 1.5 MB Rohdatei
  ...
  renderMainSection() {
    ...
    if (this._activeSection === "cardmod") return this.renderCardmods();
  ...
  renderPreviewColorMenu() {
    ...
    // v0.2.8: Live-Test-Rueckmeldung: "die vorschau läuft scheinbar über
    // card-background-color und ändert deshalb den hintergrund nicht mehr."
    // Alle Vorschau-Kartenschalen (dieses Farbmenü + Schrift-Menü + die 6
    // Mockup-Karten unten) lasen bg bisher zuerst aus "card-background-color"
    // - das Feld, das der Glaseffekt seit v0.2.7 bewusst NICHT mehr anfasst
    // (siehe generatorApplyCardTransparency). Dadurch zeigte die Vorschau nie
    // den gewählten Glaseffekt/die Transparenz, obwohl das echte Theme (ueber
    // ha-card-background) korrekt getoent war. Prioritaet in allen 8 Stellen
    // getauscht: "ha-card-background" zuerst, "card-background-color" nur
    // noch als letzter Fallback (z. B. bevor überhaupt ein Glaseffekt gewählt
    // wurde).
  ...
  render() {
    ...
    // v0.2.19: "card-mod-generator" ist keine eigene Nav-Seite mehr (siehe
    // renderNavigation()/renderCodeEditor()) - gespeicherter Zustand aus
    // aelteren Sessions faellt hierher auf den neuen Ort zurueck.
    if (this._activeSection === "card-mod-generator") this._activeSection = "code-editor";
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .cardmod-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 12px; }
        .cardmod-card { display: flex; align-items: center; gap: 10px; padding: 12px 14px; border: 1px solid var(--hatg-border); border-radius: 12px; background: var(--hatg-field); transition: border-color .15s ease; }
        .cardmod-card.is-selected { border-color: var(--hatg-blue); }
        .cardmod-card strong { flex: 1 1 auto; min-width: 0; font-size: 13px; line-height: 1.35; }
        .cardmod-system-badge { flex: 0 0 auto; width: 24px; height: 24px; border-radius: 50%; display: grid; place-items: center; color: #fff; }
        .cardmod-system-badge ha-icon { --mdc-icon-size: 14px; }
        .modal-box h3 .cardmod-system-badge { width: 26px; height: 26px; }
        .modal-box h3 .cardmod-system-badge ha-icon { --mdc-icon-size: 15px; }
        .cardmod-toggle-switch { position: relative; flex: 0 0 auto; display: inline-block; width: 42px; height: 24px; }
        .cardmod-toggle-switch input { opacity: 0; width: 0; height: 0; }
        .cardmod-toggle-slider { position: absolute; inset: 0; cursor: pointer; background: var(--hatg-border); border-radius: 999px; transition: background .15s ease; }
        .cardmod-toggle-slider::before { content: ""; position: absolute; width: 18px; height: 18px; left: 3px; top: 3px; background: #fff; border-radius: 50%; transition: transform .15s ease; }
        .cardmod-toggle-switch input:checked + .cardmod-toggle-slider { background: var(--hatg-blue); }
        .cardmod-toggle-switch input:checked + .cardmod-toggle-slider::before { transform: translateX(18px); }
        @media (max-width: 820px) {
          .cardmod-grid { grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); }
        }
  ...
  bindEvents() {
    ...
    // v0.2.22: Enrico meldete per Screenshot, dass die untere Bulk-Leiste auf
    // "Alle Felder" (Footer-Kopie unter der Feldliste, siehe renderAlleFelder())
    // nicht reagiert. Ursache: renderBulkBar() wird zweimal ausgegeben (Kopf +
    // Fuss), aber alle Handler hier nutzten querySelector() (Singular) - das
    // bindet ausschliesslich an die ERSTE (obere) Leiste im DOM, die zweite,
    // identisch aufgebaute Leiste blieb komplett unverdrahtet. Fix: pro
    // ".bulk-bar"-Instanz einzeln verdrahten (bar.querySelector(...) bleibt
    // dabei bewusst auf die jeweils eigene Leiste beschraenkt, damit z. B. die
    // Farbvorschau in der oberen Leiste nicht durch Tippen in der unteren
    // ueberschrieben wird) - beide Leisten wirken danach unabhaengig auf
    // dieselbe this._state.selectedKeys-Auswahl.
    ...
    this.shadowRoot.querySelectorAll("[data-hex-input]").forEach((el) => {
      el.addEventListener("input", () => {
        ...
        if (HATG_RGBA_RE.test(typed)) {
          // v0.2.13: rgba(...) direkt im Hex/RGB-Anzeigefeld eingegeben, ohne
          // vorher auf die RGBA-Pille zu wechseln. Bisher wurde das hier still
          // auf den reinen Hex-Anteil zusammengestaucht (Transparenz ging
          // verloren, Anzeige "sprang zurueck" auf Hex). Jetzt: kompletten
          // rgba-Wert uebernehmen und automatisch auf die RGBA-Ansicht
          // umschalten, damit sofort sichtbar ist, dass das Feld jetzt rgba
          // ist.
          if (!this._state.colorViewFormat) this._state.colorViewFormat = {};
    ...
    this.shadowRoot.querySelectorAll("[data-bg-style]").forEach((button) => {
      button.addEventListener("click", () => {
        // v0.2.20: "Bild" oeffnet den Upload-Dialog statt sofort einen Wert
        // zu committen - es gibt (anders als bei den Verlauf-Presets) noch
        // keinen Wert, bevor der Nutzer nicht tatsaechlich ein Bild waehlt.
    ...
    this.shadowRoot.querySelectorAll("[data-cardmod-info-open]").forEach((el) => {
      el.addEventListener("click", () => {
        this._state.cardmodInfoOpenId = el.getAttribute("data-cardmod-info-open");
        this.render();
      });
    });
    this.shadowRoot.querySelectorAll("[data-cardmod-info-close]").forEach((el) => {
      el.addEventListener("click", () => {
        this._state.cardmodInfoOpenId = null;
        this.render();
      });
    });
    this.shadowRoot.querySelectorAll("[data-cardmod-toggle]").forEach((el) => {
      el.addEventListener("change", () => {
        const id = el.getAttribute("data-cardmod-toggle");
        if (id) this.toggleCardmod(id);
      });
    });
  ...
  refreshPreviewCards() {
    ...
    // anzufassen. Forum-Feedback: "wenn man Werte von Hand aendert
  ...
  validateTheme() {
    ...
    // card-mod-root-yaml/card-mod-card-yaml sind seit der Cardmod-Seite (v0.2.15)
    // bewusst leer, solange kein Baustein angeschaltet ist - das ist der neue
    // Standardzustand, kein Fehler. Vorher (als card-mod-* immer einen
    // pauschalen Block enthielt) war "leer" tatsaechlich ungewoehnlich, daher
    // die generische Leer-Pruefung unten - die zaehlt fuer diese beiden Felder
    // nicht mehr als Problem.
    const allowEmpty = new Set(["card-mod-root-yaml", "card-mod-card-yaml"]);
    ["light", "dark"].forEach((mode) => {
      const values = this._state.values[mode];
