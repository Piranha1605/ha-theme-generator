...
const HATG_CARDMODS = [
  ...
  .bubble-line {
    ...
    cardCss: `/* v0.2.13: Separator-Karten (card_type: separator) sollen als flacher
     Trenner ohne erhabenen Karten-Schatten wirken. Der allgemeine ha-card-
     Schatten (bubble-box-shadow/ha-card-box-shadow) wird hier gezielt nur
     fuer Karten mit einem .bubble-separator-Kind zurueckgesetzt. */
  ha-card:has(.bubble-separator) {
...
class HATGPanel extends HTMLElement {
  ...
  renderCardmods() {
    const enabled = this.cardmodEnabledIds();
    const rows = HATG_CARDMODS.map((cardmod) => {
      const isOn = enabled.has(cardmod.id);
      return `
        <div class="cardmod-row ${isOn ? "is-selected" : ""}">
          <div class="cardmod-col-heading">
    ...
    return `
      <section class="editor-section">
        <div class="section-heading">
          <span class="eyebrow">Erweiterungen</span>
          <h1>Cardmod</h1>
          <p>Statt eines pauschalen card-mod-Blocks für alle Karten: hier einzelne CSS-Bausteine gezielt ein-/ausschalten. Aktive Bausteine landen sofort in card-mod-root-yaml/card-mod-card-yaml (Light+Dark gleichzeitig). Die ersten 5 sind bestätigte Bugfixes und standardmäßig an, der Rest ist laut A/B-Test vermutlich redundant und deshalb aus.</p>
        </div>
        <div class="plugin-list">${rows}</div>
      </section>`;
  }
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .cardmod-row { display: grid; grid-template-columns: 220px 1fr; gap: 18px; align-items: start; padding: 16px; border: 1px solid var(--hatg-border); border-radius: 14px; background: var(--hatg-field); transition: border-color .15s ease; }
        .cardmod-row.is-selected { border-color: var(--hatg-blue); }
        .cardmod-col-heading { display: flex; align-items: center; gap: 12px; min-width: 0; }
        .cardmod-col-heading strong { font-size: 13.5px; line-height: 1.35; }
        .cardmod-col-text { min-width: 0; display: flex; flex-direction: column; gap: 6px; }
        .cardmod-col-text p { margin: 0; font-size: 11.5px; color: var(--hatg-muted); line-height: 1.5; }
        .cardmod-col-text p.plugin-hint { color: var(--hatg-text-dim); }
        ...
        @media (max-width: 820px) {
          .cardmod-row { grid-template-columns: 1fr; }
        }
