...
// v0.2.26: "eigene Cardmods hinzufuegen" - eigene CSS-Bausteine, die (anders
// als reines Freitext-Tippen in card-mod-root-yaml/card-mod-card-yaml) auch
// erhalten bleiben, wenn man danach einen der eingebauten Cardmods an-/
// ausschaltet (toggleCardmod() baut die Felder sonst komplett aus der
// eingebauten Registry neu auf und wuerde freien Text verschlucken).
//
// Bewusst KEINE eigene, separat gespeicherte Registry (kein neuer
// localStorage-/Arbeitsdatei-Eintrag noetig): die Definition (Name,
// Beschreibung, CSS) wird direkt als Meta-Kommentar in card-mod-root-yaml/
// card-mod-card-yaml selbst kodiert - dieselben zwei Felder, die ohnehin
// schon ueberall (Autosave, Arbeitsdatei, Theme-Export/-Import) mitlaufen.
// Dadurch ueberleben eigene Cardmods automatisch jeden bestehenden
// Speicherweg, ohne dass irgendwo zusaetzliche Persistenz verdrahtet werden
// musste. Ein Baustein gilt als "eigen" (nicht eingebaut) genau dann, wenn
// sein Block einen META-Kommentar traegt - eingebaute Bausteine haben nie
// einen, daher ist keine Namens-/ID-Konvention noetig, um beides sauber zu
// unterscheiden.
function hatgEncodeCardmodMeta(meta) {
  const json = JSON.stringify(meta);
  if (typeof btoa === "function") return btoa(unescape(encodeURIComponent(json)));
  return Buffer.from(json, "utf8").toString("base64");
}
function hatgDecodeCardmodMeta(b64) {
  try {
...
function hatgCardmodMetaMarker(id, meta) {
  return `/* HATG:CARDMOD:${id}:META:${hatgEncodeCardmodMeta(meta)} */`;
}
function hatgCustomCardmodBlock(id, meta, css) {
  const trimmed = String(css ?? "").trim();
  if (!trimmed) return "";
  return `${hatgCardmodMarkerStart(id)}\n${hatgCardmodMetaMarker(id, meta)}\n${trimmed}\n${hatgCardmodMarkerEnd(id)}`;
}
function hatgFindCardmodBlocks(text) {
  const results = [];
  const re = /\/\* HATG:CARDMOD:([a-zA-Z0-9_-]+):START \*\/([\s\S]*?)\/\* HATG:CARDMOD:\1:END \*\//g;
  const src = String(text || "");
  let m;
  while ((m = re.exec(src)) !== null) {
    results.push({ id: m[1], fullMatch: m[0], inner: m[2] });
  }
  return results;
}
function hatgSplitCardmodBlockInner(id, inner) {
  const metaRe = new RegExp(`^\\n?/\\* HATG:CARDMOD:${id}:META:([A-Za-z0-9+/=]+) \\*/\\n?`);
  const metaMatch = metaRe.exec(inner);
  if (!metaMatch) return { meta: null, css: inner.replace(/^\n/, "").replace(/\n$/, "") };
  return { meta: hatgDecodeCardmodMeta(metaMatch[1]), css: inner.slice(metaMatch[0].length).replace(/\n$/, "") };
}
function hatgParseCustomCardmodDefs(rootText, cardText) {
  const byId = new Map();
  const absorb = (text, part) => {
    hatgFindCardmodBlocks(text).forEach((block) => {
      const { meta, css } = hatgSplitCardmodBlockInner(block.id, block.inner);
      if (!meta) return;
      const entry = byId.get(block.id) || { id: block.id, label: block.id, desc: "", rootCss: "", cardCss: "" };
      entry[part === "root" ? "rootCss" : "cardCss"] = css;
      if (meta.label) entry.label = meta.label;
      if (meta.desc) entry.desc = meta.desc;
      byId.set(block.id, entry);
    });
  };
  absorb(rootText, "root");
  absorb(cardText, "card");
  return [...byId.values()];
}
function hatgRemoveCardmodBlockById(text, id) {
  const re = new RegExp(`\\n?/\\* HATG:CARDMOD:${id}:START \\*/[\\s\\S]*?/\\* HATG:CARDMOD:${id}:END \\*/`, "g");
  return String(text || "").replace(re, "").replace(/\n{3,}/g, "\n\n");
}
function hatgExtractCustomBlocksText(text) {
  return hatgFindCardmodBlocks(text)
    .filter((block) => hatgSplitCardmodBlockInner(block.id, block.inner).meta)
    .map((block) => block.fullMatch)
    .join("\n\n");
}
function hatgSlugCardmodId(label, existingIds) {
  const cleaned = hatgTransliterateGerman(String(label || "").trim())
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  const base = "custom-" + (cleaned || "eigener-baustein");
...
const HATG_CARDMOD_SYSTEM_META = {
  ...
  custom: { icon: "mdi:account-edit-outline", label: "Eigener Cardmod", navId: "card-mod-generator" },
};
...
class HATGPanel extends HTMLElement {
  constructor() {
    ...
    this._state = {
      ...
      customCardmodDeleteConfirmId: null,
      selectMode: false,
      selectedKeys: [],
  ...
  toggleCardmod(id) {
    ...
    // Eigene Cardmods (siehe hatgExtractCustomBlocksText) haben keinen
    // Ein-/Ausschalter und sind daher nie Teil von "enabled" - sie muessen
    // hier explizit aus dem AKTUELLEN Text gerettet und hinter den frisch
    // aus der eingebauten Registry gebauten Bloecken wieder angehaengt
    // werden, sonst wuerden sie beim Umschalten eines eingebauten Cardmods
    // verloren gehen.
    const customRootExtra = hatgExtractCustomBlocksText(this._state.values.light["card-mod-root-yaml"] || "");
    const customCardExtra = hatgExtractCustomBlocksText(this._state.values.light["card-mod-card-yaml"] || "");
    const rootText = hatgBuildCardmodField(enabled, "root") + (customRootExtra ? "\n\n" + customRootExtra + "\n" : "");
    const cardText = hatgBuildCardmodField(enabled, "card") + (customCardExtra ? "\n\n" + customCardExtra + "\n" : "");
  ...
  customCardmodDefs() {
    const values = this._state.values.light;
    return hatgParseCustomCardmodDefs(values["card-mod-root-yaml"] || "", values["card-mod-card-yaml"] || "");
  }

  openCustomCardmodDialog(id) {
    if (id) {
      const def = this.customCardmodDefs().find((d) => d.id === id);
      if (!def) return;
      this._state.customCardmodDialog = { mode: "edit", id: def.id, label: def.label, desc: def.desc, rootCss: def.rootCss, cardCss: def.cardCss };
    } else {
      this._state.customCardmodDialog = { mode: "add", id: null, label: "", desc: "", rootCss: "", cardCss: "" };
    }
    this.render();
  }

  closeCustomCardmodDialog() {
    this._state.customCardmodDialog = null;
    this.render();
  }

  saveCustomCardmodDialog() {
    const dialog = this._state.customCardmodDialog;
    if (!dialog) return;
    const root = this.shadowRoot;
    const label = (root.querySelector("[data-cc-label]")?.value || "").trim();
    const desc = (root.querySelector("[data-cc-desc]")?.value || "").trim();
    const rootCss = (root.querySelector("[data-cc-root-css]")?.value || "").trim();
    const cardCss = (root.querySelector("[data-cc-card-css]")?.value || "").trim();
    if (!label) {
      this.showToast("Bitte einen Namen für den Cardmod eintragen.");
      return;
    }
    if (!rootCss && !cardCss) {
      this.showToast("Bitte mindestens ein CSS-Feld (root-yaml oder card-yaml) ausfüllen.");
      return;
    }
    const existingIds = new Set(HATG_CARDMODS.map((c) => c.id).concat(this.customCardmodDefs().map((d) => d.id)));
    let id = dialog.id;
    if (id) {
      existingIds.delete(id);
    } else {
      id = hatgSlugCardmodId(label, existingIds);
    }
    const meta = { label, desc };
    ["light", "dark"].forEach((mode) => {
      let rootText = this._state.values[mode]["card-mod-root-yaml"] || "";
      let cardText = this._state.values[mode]["card-mod-card-yaml"] || "";
      if (dialog.id) {
        rootText = hatgRemoveCardmodBlockById(rootText, dialog.id);
        cardText = hatgRemoveCardmodBlockById(cardText, dialog.id);
      }
      const rootBlock = hatgCustomCardmodBlock(id, meta, rootCss);
      const cardBlock = hatgCustomCardmodBlock(id, meta, cardCss);
      if (rootBlock) rootText = (rootText.trim() ? rootText.trim() + "\n\n" : "") + rootBlock + "\n";
      if (cardBlock) cardText = (cardText.trim() ? cardText.trim() + "\n\n" : "") + cardBlock + "\n";
      this._state.values[mode]["card-mod-root-yaml"] = rootText;
      this._state.values[mode]["card-mod-card-yaml"] = cardText;
      this._state.source[mode]["card-mod-root-yaml"] = "custom";
      this._state.source[mode]["card-mod-card-yaml"] = "custom";
    ...
    this._state.customCardmodDialog = null;
    this.render();
    this.showToast(dialog.mode === "edit" ? `Eigener Cardmod "${label}" aktualisiert.` : `Eigener Cardmod "${label}" hinzugefügt.`);
  }

  requestDeleteCustomCardmod(id) {
    this._state.customCardmodDeleteConfirmId = id;
    this.render();
  }

  cancelDeleteCustomCardmod() {
    this._state.customCardmodDeleteConfirmId = null;
    this.render();
  }

  confirmDeleteCustomCardmod() {
    const id = this._state.customCardmodDeleteConfirmId;
    if (!id) return;
    const def = this.customCardmodDefs().find((d) => d.id === id);
    ["light", "dark"].forEach((mode) => {
      this._state.values[mode]["card-mod-root-yaml"] = hatgRemoveCardmodBlockById(this._state.values[mode]["card-mod-root-yaml"] || "", id);
      this._state.values[mode]["card-mod-card-yaml"] = hatgRemoveCardmodBlockById(this._state.values[mode]["card-mod-card-yaml"] || "", id);
      this._state.source[mode]["card-mod-root-yaml"] = "custom";
      this._state.source[mode]["card-mod-card-yaml"] = "custom";
    });
    this._state.customCardmodDeleteConfirmId = null;
    this.render();
    this.showToast(def ? `Eigener Cardmod "${def.label}" gelöscht.` : "Eigener Cardmod gelöscht.");
  }
  ...
  renderCardmods() {
    ...
    const customDefs = this.customCardmodDefs();
    const customCards = customDefs
      .map((def) => {
        const sysMeta = HATG_CARDMOD_SYSTEM_META.custom;
        return `
        <div class="cardmod-card cardmod-card-custom is-selected">
          <span class="cardmod-system-badge" style="background: ${hatgNavIconGradient(sysMeta.navId)};" title="${hatgEscape(sysMeta.label)}" aria-label="${hatgEscape(sysMeta.label)}">
            <ha-icon icon="${sysMeta.icon}"></ha-icon>
          </span>
          <strong>${hatgEscape(def.label)}</strong>
          <div class="cardmod-custom-actions">
            <button type="button" class="plugin-info-button" data-cardmod-custom-edit="${def.id}" title="Bearbeiten" aria-label="Bearbeiten">
              <ha-icon icon="mdi:pencil-outline"></ha-icon>
            </button>
            <button type="button" class="plugin-info-button" data-cardmod-custom-delete="${def.id}" title="Löschen" aria-label="Löschen">
    ...
    return `
      <section class="editor-section">
        ...
        <div class="section-heading cardmod-custom-heading">
          <h2>Eigene Cardmods</h2>
          <p>Eigenes CSS für card-mod-root-yaml/card-mod-card-yaml – bleibt, anders als reines Freitext-Tippen im Code-Editor, auch beim Ein-/Ausschalten der eingebauten Bausteine oben erhalten.</p>
          <button type="button" class="modal-btn ghost" data-cardmod-custom-add>
            <ha-icon icon="mdi:plus"></ha-icon> Eigenen Cardmod hinzufügen
          </button>
        </div>
        ${customDefs.length ? `<div class="cardmod-grid">${customCards}</div>` : `<p class="empty-hint">Noch keine eigenen Cardmods – über den Button oben hinzufügen.</p>`}
      </section>`;
  }
  ...
  renderCustomCardmodDialog() {
    ...
    return `
      <div class="modal-scrim" data-cc-cancel></div>
      <div class="modal-box modal-box-wide">
        <h3>${isEdit ? "Eigenen Cardmod bearbeiten" : "Eigenen Cardmod hinzufügen"}</h3>
        <p class="plugin-hint">CSS für card-mod-root-yaml wirkt dashboardweit (z. B. Regeln, die die ganze Seite betreffen), CSS für card-mod-card-yaml gilt nur innerhalb einzelner Karten – das ist der Normalfall für die meisten Anpassungen. Mindestens eines der beiden Felder ausfüllen.</p>
        <label class="modal-field">
          <span>Name</span>
          <input type="text" value="${hatgEscape(dialog.label)}" data-cc-label placeholder="z. B. Mein Slider-Glow" />
        </label>
        <label class="modal-field">
          <span>Beschreibung (optional)</span>
          <input type="text" value="${hatgEscape(dialog.desc)}" data-cc-desc placeholder="Kurze Notiz, wofür das ist" />
        </label>
        <label class="modal-field">
          <span>CSS für card-mod-root-yaml (optional)</span>
          <textarea class="field-textarea" spellcheck="false" rows="6" data-cc-root-css placeholder=".bubble-beispiel { ... }">${hatgEscape(dialog.rootCss)}</textarea>
        </label>
        <label class="modal-field">
          <span>CSS für card-mod-card-yaml (optional)</span>
          <textarea class="field-textarea" spellcheck="false" rows="6" data-cc-card-css placeholder="ha-card { ... }">${hatgEscape(dialog.cardCss)}</textarea>
        </label>
        <div class="modal-actions">
          <button type="button" class="modal-btn ghost" data-cc-cancel>Abbrechen</button>
          <button type="button" class="modal-btn primary" data-cc-save>${isEdit ? "Speichern" : "Hinzufügen"}</button>
        </div>
      </div>`;
  }

  renderCustomCardmodDeleteConfirm() {
    const id = this._state.customCardmodDeleteConfirmId;
    if (!id) return "";
    const def = this.customCardmodDefs().find((d) => d.id === id);
    const label = def ? def.label : id;
    return `
      <div class="modal-scrim" data-cc-delete-cancel></div>
      <div class="modal-box">
        <h3>Eigenen Cardmod löschen?</h3>
        <p>"${hatgEscape(label)}" wird aus card-mod-root-yaml/card-mod-card-yaml entfernt. Das lässt sich nicht rückgängig machen, du kannst ihn aber jederzeit neu anlegen.</p>
        <div class="modal-actions">
          <button type="button" class="modal-btn ghost" data-cc-delete-cancel>Abbrechen</button>
          <button type="button" class="modal-btn primary" data-cc-delete-confirm>Löschen</button>
        </div>
      </div>`;
  }
      ...
      HATG_MANIFEST.sections.forEach((s) => {
        ...
        groups.forEach((g) => {
          ...
          if (g.keys.includes(HATG_CUSTOM_YAML_KEY)) {
            // "eigene-theme-eintraege" ist kein echtes Theme-Feld, sondern ein
            // Freitext-Sammelfeld: statt es wie jedes andere Feld als
            // "eigene-theme-eintraege: '...'" zu exportieren, wird sein Inhalt
            // zeilenweise ALS ROHE, EIGENSTAENDIGE YAML-Zeilen direkt in den
            // modes:-Block eingefuegt - so werden vom Nutzer frei getippte
            // key: value-Paare zu echten Geschwister-Theme-Variablen.
            const raw = String(values[HATG_CUSTOM_YAML_KEY] || "").trim();
            if (raw) {
              const customLines = raw
                .split("\n")
                .map((line) => line.trim())
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .cardmod-custom-heading { margin: 30px 0 14px; }
        .cardmod-custom-heading h2 { font-size: 17px; font-weight: 700; margin: 0 0 6px; }
        .cardmod-custom-heading p { margin: 0 0 12px; color: var(--hatg-muted); font-size: 12.5px; line-height: 1.5; }
        .cardmod-custom-actions { display: flex; gap: 4px; flex: 0 0 auto; }
        @media (max-width: 820px) {
      ...
      ${this.renderCustomCardmodDeleteConfirm()}
  ...
  bindEvents() {
    ...
    this.shadowRoot.querySelector("[data-cardmod-custom-add]")?.addEventListener("click", () => {
      this.openCustomCardmodDialog(null);
    });
    this.shadowRoot.querySelectorAll("[data-cardmod-custom-edit]").forEach((el) => {
      el.addEventListener("click", () => {
        this.openCustomCardmodDialog(el.getAttribute("data-cardmod-custom-edit"));
      });
    });
    this.shadowRoot.querySelectorAll("[data-cardmod-custom-delete]").forEach((el) => {
      el.addEventListener("click", () => {
        this.requestDeleteCustomCardmod(el.getAttribute("data-cardmod-custom-delete"));
      });
    });
    this.shadowRoot.querySelectorAll("[data-cc-cancel]").forEach((el) => {
      el.addEventListener("click", () => this.closeCustomCardmodDialog());
    });
    this.shadowRoot.querySelector("[data-cc-save]")?.addEventListener("click", () => this.saveCustomCardmodDialog());
    this.shadowRoot.querySelectorAll("[data-cc-delete-cancel]").forEach((el) => {
      el.addEventListener("click", () => this.cancelDeleteCustomCardmod());
    });
    this.shadowRoot.querySelector("[data-cc-delete-confirm]")?.addEventListener("click", () => this.confirmDeleteCustomCardmod());
  ...
  validateTheme() {
    ...
    const allowEmpty = new Set(["card-mod-root-yaml", "card-mod-card-yaml", HATG_CUSTOM_YAML_KEY]);
    ["light", "dark"].forEach((mode) => {
      const values = this._state.values[mode];
