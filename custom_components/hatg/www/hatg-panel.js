...
// v0.2.28: "Ziel-Element" fuer eigene Cardmods - manche card-mod-Schnipsel
// (z. B. aus dem Forum kopiert) nutzen nicht flaches CSS, sondern card-mods
// spezielle verschachtelte YAML-Schreibweise ("elementname$: |  ...") um in
// ein VERSCHACHTELTES Shadow-DOM-Element hineinzustyeln (z. B.
// "mushroom-shape-icon$:" fuer das Icon-Shape innerhalb einer Mushroom-
// Karte). Wird so ein Block einfach als flacher Text neben die anderen
// (echten Flach-CSS-)Bausteine gehaengt, ist das gesamte Feld kein gueltiges
// YAML mehr (nachgewiesen per Test) und card-mod faellt komplett auf reinen
// Flach-CSS-Modus zurueck - der Verschachtelungs-Effekt verschwindet
// stillschweigend. hatgAssembleCardmodField() loest das: sobald IRGENDEIN
// aktiver Baustein ein "target" hat, wird das gesamte Feld stattdessen als
// gueltiges YAML mit einem "$: |"-Schluessel fuer die normalen Flach-CSS-
// Bloecke plus einem eigenen "<target>$: |"-Schluessel je Ziel-Element
// zusammengesetzt. Ohne "target" (der ueberwiegende Normalfall) bleibt das
// Verhalten exakt wie zuvor (reines Flach-CSS, byte-identisch).
function hatgIndentText(text, spaces) {
...
function hatgDedentCardmodInner(text) {
  const raw = String(text || "");
  const lines = raw.split("\n");
  const nonEmpty = lines.filter((l) => l.trim().length > 0);
  if (!nonEmpty.length) return raw;
  const minIndent = Math.min(...nonEmpty.map((l) => (l.match(/^[ \t]*/) || [""])[0].length));
  if (!minIndent) return raw;
  return lines.map((l) => (l.length >= minIndent ? l.slice(minIndent) : l)).join("\n");
}
function hatgSplitCardmodBlockInner(id, inner) {
  const dedented = hatgDedentCardmodInner(inner);
  const metaRe = new RegExp(`^\\n?/\\* HATG:CARDMOD:${id}:META:([A-Za-z0-9+/=]+) \\*/\\n?`);
  const metaMatch = metaRe.exec(dedented);
  if (!metaMatch) return { meta: null, css: dedented.replace(/^\n/, "").replace(/\n$/, "") };
  return { meta: hatgDecodeCardmodMeta(metaMatch[1]), css: dedented.slice(metaMatch[0].length).replace(/\n$/, "") };
}
function hatgParseCustomCardmodDefs(rootText, cardText) {
  ...
  const absorb = (text, part) => {
    hatgFindCardmodBlocks(text).forEach((block) => {
      ...
      if (!meta) return;
      const entry = byId.get(block.id) || { id: block.id, label: block.id, desc: "", rootCss: "", cardCss: "", target: "" };
      entry[part === "root" ? "rootCss" : "cardCss"] = css;
      if (meta.label) entry.label = meta.label;
      if (meta.desc) entry.desc = meta.desc;
      if (meta.target) entry.target = meta.target;
...
function hatgAssembleCardmodField(enabledIds, customDefs, part) {
  const builtinBlocks = HATG_CARDMODS.filter((c) => enabledIds.has(c.id))
    .map((c) => hatgCardmodBlock(c, part))
    .filter((b) => b);
  const customBlocks = (customDefs || [])
    .map((d) => {
      const css = part === "root" ? d.rootCss : d.cardCss;
      if (!String(css || "").trim()) return null;
      const meta = { label: d.label, desc: d.desc };
      if (d.target) meta.target = d.target;
      return { target: d.target ? String(d.target).trim() : "", text: hatgCustomCardmodBlock(d.id, meta, css) };
    })
    .filter((b) => b);

  const flatCustom = customBlocks.filter((b) => !b.target).map((b) => b.text);
  const targeted = customBlocks.filter((b) => b.target);
  const flatAll = [hatgCardmodHeaderComment(part), "", ...builtinBlocks, ...flatCustom].join("\n\n");
  ...
  targeted.forEach((b) => {
    ...
    arr.push(b.text);
    byTarget.set(b.target, arr);
  });

  const sections = [`$: |\n${hatgIndentText(flatAll, 2)}`];
  byTarget.forEach((texts, target) => {
    sections.push(`${target}$: |\n${hatgIndentText(texts.join("\n\n"), 2)}`);
  });
...
class HATGPanel extends HTMLElement {
  ...
  // v0.2.28: zentrale Zusammenbau-Stelle fuer beide Felder - liest eigene
  // Cardmods per Default aus dem AKTUELLEN Text (this.customCardmodDefs()),
  // kann aber ueber den zweiten Parameter mit einer schon geaenderten Liste
  // ueberschrieben werden (Speichern/Loeschen kennen die neue Liste ja schon,
  // bevor sie geschrieben wird). hatgAssembleCardmodField() erkennt dabei
  // automatisch, ob ein "Ziel-Element" (target) im Spiel ist und waehlt die
  // passende Zusammensetzung (flach oder $-Wrapper), siehe dort.
  rebuildCardmodFields(enabledIds, customDefs) {
    const defs = customDefs || this.customCardmodDefs();
    const rootText = hatgAssembleCardmodField(enabledIds, defs, "root");
    const cardText = hatgAssembleCardmodField(enabledIds, defs, "card");
  ...
  toggleCardmod(id) {
    ...
    this.rebuildCardmodFields(enabled);
    this.render();
  }
  ...
  openCustomCardmodDialog(id) {
    if (id) {
      ...
      if (!def) return;
      this._state.customCardmodDialog = { mode: "edit", id: def.id, label: def.label, desc: def.desc, rootCss: def.rootCss, cardCss: def.cardCss, target: def.target || "" };
    } else {
      this._state.customCardmodDialog = { mode: "add", id: null, label: "", desc: "", rootCss: "", cardCss: "", target: "" };
    }
    this.render();
  }
  ...
  saveCustomCardmodDialog() {
    ...
    const target = (root.querySelector("[data-cc-target]")?.value || "").trim();
    ...
    const existingDefs = this.customCardmodDefs();
    const existingIds = new Set(HATG_CARDMODS.map((c) => c.id).concat(existingDefs.map((d) => d.id)));
    let id = dialog.id;
    if (id) {
    ...
    const newDef = { id, label, desc, rootCss, cardCss, target };
    const nextDefs = dialog.id ? existingDefs.map((d) => (d.id === dialog.id ? newDef : d)) : existingDefs.concat([newDef]);
    this.rebuildCardmodFields(this.cardmodEnabledIds(), nextDefs);
    this._state.customCardmodDialog = null;
    this.render();
  ...
  confirmDeleteCustomCardmod() {
    ...
    if (!id) return;
    const existingDefs = this.customCardmodDefs();
    const def = existingDefs.find((d) => d.id === id);
    const nextDefs = existingDefs.filter((d) => d.id !== id);
    this.rebuildCardmodFields(this.cardmodEnabledIds(), nextDefs);
    this._state.customCardmodDeleteConfirmId = null;
    this.render();
  ...
  renderCustomCardmodDialog() {
    ...
    return `
      ...
      <div class="modal-box modal-box-wide">
        ...
        <label class="modal-field">
          <span>Ziel-Element (optional, nur für Sonderfälle)</span>
          <input type="text" value="${hatgEscape(dialog.target || "")}" data-cc-target placeholder="z. B. mushroom-shape-icon" />
        </label>
        <p class="plugin-hint">Nur ausfüllen, wenn dein CSS eigentlich in ein VERSCHACHTELTES Element hinein soll (z. B. das Icon-Innenleben einer Mushroom-Karte) – card-mod nennt das "mushroom-shape-icon$" o. ä. Hier reicht der reine Name ohne "$", HATG ergänzt den Rest automatisch. Für die meisten Cardmods bleibt dieses Feld leer.</p>
        <label class="modal-field">
