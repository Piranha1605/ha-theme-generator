...
function hatgExtractCardmodBlock(text, id) {
  const start = hatgRegexEscape(hatgCardmodMarkerStart(id));
  const end = hatgRegexEscape(hatgCardmodMarkerEnd(id));
  const m = new RegExp(`${start}\\n([\\s\\S]*?)\\n\\s*${end}`).exec(String(text || ""));
...
const HATG_STATUS_UNAVAILABLE_KEYS = ["state-unavailable-color", "state-icon-unavailable-color"];

const HATG_STATUS_AN_BUBBLE_KEYS = [
  "bubble-accent-color",
  "bubble-active-color",
  "bubble-toggle-color",
...
const HATG_STATUS_AN_MUSH_KEYS = [
...
const HATG_STATUS_UNAVAILABLE_MUSH_KEYS = ["mush-rgb-disabled"];

const HATG_STATUS_LABELS = { an: "An", aus: "Aus", unavailable: "Nicht verfügbar" };
const HATG_STATUS_FRAMEWORK_KEYS = {
  an: { bubble: HATG_STATUS_AN_BUBBLE_KEYS, mush: HATG_STATUS_AN_MUSH_KEYS },
  aus: { bubble: [], mush: [] },
  unavailable: { bubble: [], mush: HATG_STATUS_UNAVAILABLE_MUSH_KEYS },
};

const HATG_ICON_FARBE_KEYS = [
  "state-icon-color",
...
const HATG_ICON_HINTERGRUND_KEYS = [
...
const HATG_ICON_MASTER_GROUPS = {
  farbe: { keys: HATG_ICON_FARBE_KEYS, label: "Icon-Farbe" },
  hintergrund: { keys: HATG_ICON_HINTERGRUND_KEYS, label: "Icon-Hintergrund" },
};
...
function hatgIsLong(key, value) {
  ...
  if (key === "card-mod-root") return true;
...
class HATGPanel extends HTMLElement {
  ...
  renderStatusFrameworkWidget(group) {
    const ziele = HATG_STATUS_FRAMEWORK_KEYS[group];
    if (!ziele || (!ziele.bubble.length && !ziele.mush.length)) return "";
    const an = this.statusFrameworkSyncOn(group);
    const anzahl = ziele.bubble.length + ziele.mush.length;
    const titel = an
    ...
    return `
      <span class="status-fw-widget ${an ? "on" : ""}" title="${hatgEscape(titel)}">
        <ha-icon icon="mdi:home-assistant"></ha-icon>
        ${ziele.bubble.length ? '<ha-icon icon="mdi:chart-bubble"></ha-icon>' : ""}
        ${ziele.mush.length ? '<ha-icon icon="mdi:mushroom-outline"></ha-icon>' : ""}
  ...
  renderStatusMasterSection() {
    ...
    return `
      <div class="editing-surface ${dark ? "editing-surface-dark" : ""}">
        <div class="field-group-title">Status-Farben (An / Aus / Nicht verfügbar)</div>
        <div class="field-list">
          ${this.renderStatusMasterRow("an", "An")}
          ${this.renderStatusMasterRow("aus", "Aus")}
          ${this.renderStatusMasterRow("unavailable", "Nicht verfügbar")}
        </div>
      </div>`;
  }
  ...
  getIconMasterKeys(group) {
    return (HATG_ICON_MASTER_GROUPS[group] || HATG_ICON_MASTER_GROUPS.farbe).keys;
  }

  getIconMasterRawValue(group, mode) {
    const keys = this.getIconMasterKeys(group);
    const values = mode ? this._state.values[mode] : this.currentValues();
    return values[keys[0]];
  }

  applyIconMasterColor(group, value) {
    this.getIconMasterKeys(group).forEach((key) => this.commitField(key, value));
    this.applyPreviewTheme();
  }

  copyIconMasterToOtherMode(group) {
    const currentMode = this._state.editorMode;
    const otherMode = currentMode === "light" ? "dark" : "light";
    const value = this.getIconMasterRawValue(group, currentMode);
    this._state.editorMode = otherMode;
    this.getIconMasterKeys(group).forEach((key) => this.commitField(key, value));
    this._state.editorMode = currentMode;
    this.render();
    this.showToast(
      `${HATG_ICON_MASTER_GROUPS[group].label} nach ${otherMode === "dark" ? "Dark" : "Light"} übernommen: ${value}.`
    );
  }

  renderIconMasterRow(group) {
    const meta = HATG_ICON_MASTER_GROUPS[group];
    const anzahl = meta.keys.length;
    const raw = this.getIconMasterRawValue(group);
    ...
    return `
      <div class="field-item">
        <div class="field-head">
          <span class="field-key">${hatgEscape(meta.label)}<small class="field-section-tag">${anzahl} Felder</small></span>
          <button class="apply-button copy-mode-button" type="button" data-icon-master-copy="${group}" aria-label="Nach ${otherLabel} übernehmen" title="${hatgEscape(meta.label)} nach ${otherLabel} übernehmen">
            <ha-icon icon="mdi:swap-horizontal"></ha-icon>
          </button>
        </div>
        <div class="field-row" data-icon-master-row="${group}">
          <span class="color-dot" style="background:${parsed.hex}"></span>
          <div class="color-input-wrap">
            <input class="native-color" type="color" value="${parsed.hex}" data-icon-master-picker="${group}" aria-label="${hatgEscape(meta.label)}" />
            <input class="hex-input" type="text" value="${hatgEscape(String(raw ?? ""))}" spellcheck="false" data-icon-master-hex="${group}" aria-label="${hatgEscape(meta.label)} Farbcode" />
          </div>
        </div>
      </div>`;
  }

  renderIconMasterSection() {
    const dark = this._state.editorMode === "dark";
    return `
      <div class="editing-surface ${dark ? "editing-surface-dark" : ""}">
        <div class="field-group-title">Icons (Farbe &amp; Hintergrund)</div>
        <p class="icon-master-hint">Setzt gebündelt alle Icon-Felder von Home Assistant, Bubble Card und Mushroom - An- und Aus-Zustand bewusst identisch, damit Icons überall gleich aussehen.</p>
        <div class="field-list">
          ${this.renderIconMasterRow("farbe")}
          ${this.renderIconMasterRow("hintergrund")}
        </div>
      </div>`;
  }
  ...
  refreshCardmodTemplates(options = {}) {
    const targetKey = "card-mod-card";
    const currentMode = this._state.editorMode;
    let anzahl = 0;
    ["light", "dark"].forEach((mode) => {
  ...
  renderCardmodTemplates() {
    ...
    const hinweis = veraltet.length
      ? `<div class="cardmod-stale-bar">
          ...
          <button type="button" class="cardmod-stale-button" data-refresh-cardmod-templates>
  ...
  bindEvents() {
    ...
    this.shadowRoot.querySelectorAll("[data-icon-master-picker]").forEach((input) => {
      const commit = () => this.applyIconMasterColor(input.dataset.iconMasterPicker, input.value.toUpperCase());
      input.addEventListener("input", () => { commit(); this.render(); });
      input.addEventListener("change", () => { commit(); this.render(); });
    });
    this.shadowRoot.querySelectorAll("[data-icon-master-hex]").forEach((input) => {
      const commit = () => {
        const group = input.dataset.iconMasterHex;
        const typed = String(input.value || "").trim();
        if (!typed) return;
        this.applyIconMasterColor(group, typed);
        this.render();
      };
      input.addEventListener("change", commit);
      input.addEventListener("blur", commit);
    });
    this.shadowRoot.querySelectorAll("[data-icon-master-copy]").forEach((el) => {
      el.addEventListener("click", () => this.copyIconMasterToOtherMode(el.dataset.iconMasterCopy));
    });
    ...
    this.shadowRoot.querySelectorAll("[data-refresh-cardmod-templates]").forEach((el) => {
      el.addEventListener("click", () => this.refreshCardmodTemplates());
    });
  ...
  getStatusMasterKeys(group) {
    if (group === "aus") return HATG_STATUS_AUS_KEYS;
    if (group === "unavailable") return HATG_STATUS_UNAVAILABLE_KEYS;
    return HATG_STATUS_AN_KEYS;
  }
  ...
  applyStatusMasterColor(group, value) {
    this.getStatusMasterKeys(group).forEach((key) => this.commitField(key, value));
  ...
  applyStatusFrameworkSync(group, value, mode) {
    ...
    let n = 0;
    ziele.bubble.forEach((key) => { this.commitField(key, value); n++; });
    ziele.mush.forEach((key) => { this.commitField(key, hatgHexToRgbTriple(hex)); n++; });
  ...
  toggleStatusFrameworkSync(group) {
    ...
    this.render();
    const meta = HATG_STATUS_FRAMEWORK_KEYS[group];
  ...
  copyStatusMasterToOtherMode(group) {
    ...
    this._state.editorMode = currentMode;
    this.render();
    const label = HATG_STATUS_LABELS[group] || group;
  ...
  migrateLegacyCardmodKeys(parsed) {
    const map = { "card-mod-card-yaml": "card-mod-card", "card-mod-root-yaml": "card-mod-root" };
    let migrated = 0;
    ["light", "dark"].forEach((mode) => {
      const extra = parsed.extra ? parsed.extra[mode] : null;
      Object.keys(map).forEach((oldKey) => {
        const newKey = map[oldKey];
        [parsed[mode], extra].forEach((bag) => {
          if (!bag || bag[oldKey] === undefined) return;
          const value = String(bag[oldKey] ?? "").trim();
          delete bag[oldKey];
          if (!value) return;
  ...
  applyImportedTheme(parsed) {
    const migratedCardmod = this.migrateLegacyCardmodKeys(parsed);
    ...
    if (migratedCardmod) parts.push(`Cardmod aus altem "-yaml"-Feld auf card-mod-card/card-mod-root umgestellt (wirkt jetzt erst)`);
    const aufgefrischt = this.refreshCardmodTemplates({ silent: true });
