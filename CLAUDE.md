...
├── const.py              Konstanten
├── manifest.json         Domain hatg, aktuell v1.3.1, im Test 1.3.2b1
...
- **Ein Pfad darf nicht mit `$$` beginnen.** `$$` ist die rekursive, Shadow-DOM-durchdringende Suche; als *erster* Schritt eines Pfades legt sie UIX 8.2.0 komplett still — für die ganze Sitzung kommt dann keine einzige Vorlage mehr an, auch nicht in anderen Stilzielen. Am 2026-09-20 zweimal gemessen: mit `"$$ ha-md-list $"` blieb sogar die Seitenleiste ohne CSS (`uix-node` leer), mit `"ha-config-integration-page $$ ha-md-list $"` lief alles. In der Mitte eines Pfades ist `$$` in Ordnung und spart die Zwischenschritte.
