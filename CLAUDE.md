...
- **Ein Pfad trifft nur eine Ebene tief.** `"ha-switch $"` findet nichts, sobald das Element tiefer unter der Karte hängt — in eigenen Karten sitzt der Schalter unter `button > div > div > ha-card`. Wo ein Web-Awesome-Element CSS-Teile nach außen gibt, ist `::part()` der bessere Weg: Es greift durch die Shadow-Grenze, unabhängig von der Tiefe, und braucht kein `-yaml`-Feld. `ha-switch` bietet `base`, `control` und `thumb`.
